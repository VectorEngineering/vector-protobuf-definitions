...
* Every proto directory should have a `README.md` describing its content. See
  for example [vector.service](vector/service/README.md).
...
* Use a `[#not-implemented-hide:]` `protodoc` annotation in comments for fields that lack Vector
  implementation. These indicate that the entity is not implemented in Vector and the entity
  should be hidden from the Vector documentation.
...
- `vector.extensions` contains all definitions for the extensions, the package should match the structure of the `source` directory.
- `vector.service` contains gRPC definitions of supporting services and top-level messages for the services.
e.g. `vector.service.route.v3` contains RDS, `vector.service.listener.v3` contains LDS.
- `vector.config` contains other definitions for service configuration, bootstrap and some legacy core types.
- `vector.data` contains data format declaration for data types that Vector produces.
- `vector.type` contains common protobuf types such as percent, range and matchers.

Extensions should use the regular hierarchy. For example, configuration for network filters belongs
in a package under `vector.extensions.filter.network`.
...
1. Place the v3 extension configuration `.proto` in `api/vector/extensions` or `api/contrib/vector/extensions`, e.g.
   `api/vector/extensions/filters/http/foobar/v3/foobar.proto` together with an initial BUILD file:
   ```bazel
   load("@vector_api//bazel:api_build_system.bzl", "api_proto_package")
...
1. Update [source/extensions/extensions_metadata.yaml](../source/extensions/extensions_metadata.yaml) or [contrib/extensions_metadata.yaml](../contrib/extensions_metadata.yaml)
   ...
   * Any extension category added to `extensions_metadata.yaml` should be annotated in precisely one proto file, associated with a field of a proto message. e.g.
     ...
     message SomeMessage {
       ...
       // [#extension-category: vector.http.filters]
   ...
   * Each extension added to `extensions_metadata.yaml` should have precisely one proto file annotated with the extension name. e.g.
     ...
     // [#extension: vector.http.filters.your_new_filter]

     // VectorFilterConfig is the configuration for a VectorFilter (write real documentation here).
...
1. See the [key-value-store PR](https://github.com/vectordotdev/vector/pull/17745/files) for an example of adding a new extension point to common.

## API annotations

A number of annotations are used in the Vector APIs to provide additional API
...
* `[vector.annotations.disallowed_by_default = true]` to denote fields that have
...
### Service level
* `option (vector.annotations.resource).type = "<resource type name>";` to denote
...
* The xDS APIs are directionally client and server neutral. While many aspects
  of the APIs reflect the history of their origin as Vector's control plane APIs,
...
* Namespaces for extensions, metadata, etc. use a reverse DNS naming scheme,
  ...
  with client name, e.g. `vector.foo`, `grpc.bar`.