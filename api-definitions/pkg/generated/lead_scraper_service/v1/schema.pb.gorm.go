...
type OrganizationORM struct {
	...
	BillingEmail            string
	BillingPlan             string
	ComplianceFrameworks    pq.StringArray `gorm:"type:text[]"`
	CreatedAt               *time.Time
	CreatedBy               string
	...
	SsoProvider             string
	Status                  string
	Subscriptions           *SubscriptionORM `gorm:"foreignKey:OrganizationId;references:Id"`
	TaxId                   string
	TechnicalEmail          string
	Tenants                 []*TenantORM `gorm:"foreignKey:OrganizationId;references:Id"`
	TotalStorageLimit       int64
	UpdatedAt               *time.Time
	Website                 string
}
...
func (m *Organization) ToORM(ctx context.Context) (OrganizationORM, error) {
	...
	to.TechnicalEmail = m.TechnicalEmail
	to.Phone = m.Phone
	to.Address = m.Address
	to.SsoEnabled = m.SsoEnabled
	to.SsoProvider = m.SsoProvider
	to.SsoDomain = m.SsoDomain
	if m.AllowedDomains != nil {
		to.AllowedDomains = make(pq.StringArray, len(m.AllowedDomains))
		copy(to.AllowedDomains, m.AllowedDomains)
	}
	to.Enforce_2Fa = m.Enforce_2Fa
	to.BillingPlan = Organization_BillingPlan_name[int32(m.BillingPlan)]
	to.BillingCurrency = m.BillingCurrency
	to.AutoBilling = m.AutoBilling
	to.TaxId = m.TaxId
	if m.ComplianceFrameworks != nil {
		to.ComplianceFrameworks = make(pq.StringArray, len(m.ComplianceFrameworks))
		copy(to.ComplianceFrameworks, m.ComplianceFrameworks)
	}
	to.DataProcessingAgreement = m.DataProcessingAgreement
	to.DataRegion = m.DataRegion
	to.MaxTenants = m.MaxTenants
	to.TotalStorageLimit = m.TotalStorageLimit
	to.MaxApiKeys = m.MaxApiKeys
	to.MaxUsers = m.MaxUsers
	for _, v := range m.Tenants {
		if v != nil {
			if tempTenants, cErr := v.ToORM(ctx); cErr == nil {
				to.Tenants = append(to.Tenants, &tempTenants)
			} else {
				return to, cErr
			}
		} else {
			to.Tenants = append(to.Tenants, nil)
		}
	}
	if m.Subscriptions != nil {
		tempSubscriptions, err := m.Subscriptions.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.Subscriptions = &tempSubscriptions
	}
	if m.CreatedAt != nil {
...
func (m *OrganizationORM) ToPB(ctx context.Context) (Organization, error) {
	...
	to.Enforce_2Fa = m.Enforce_2Fa
	to.BillingPlan = Organization_BillingPlan(Organization_BillingPlan_value[m.BillingPlan])
	to.BillingCurrency = m.BillingCurrency
	to.AutoBilling = m.AutoBilling
	to.TaxId = m.TaxId
	if m.ComplianceFrameworks != nil {
	...
	for _, v := range m.Tenants {
		if v != nil {
			if tempTenants, cErr := v.ToPB(ctx); cErr == nil {
				to.Tenants = append(to.Tenants, &tempTenants)
			} else {
				return to, cErr
			}
	...
	if m.Subscriptions != nil {
		tempSubscriptions, err := m.Subscriptions.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.Subscriptions = &tempSubscriptions
...
type TenantORM struct {
	Accounts             []*AccountORM  `gorm:"foreignKey:TenantId;references:Id"`
	AllowedOrigins       pq.StringArray `gorm:"type:text[]"`
	ApiBaseUrl           string
	ApiKeys              []*TenantAPIKeyORM `gorm:"foreignKey:TenantId;references:Id"`
...
// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *Tenant) ToORM(ctx context.Context) (TenantORM, error) {
	to := TenantORM{}
	var err error
	if prehook, ok := interface{}(m).(TenantWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.DisplayName = m.DisplayName
	to.Description = m.Description
	to.ApiBaseUrl = m.ApiBaseUrl
	// Repeated type EnvironmentVariablesEntry is not an ORMable message type
	if m.AllowedOrigins != nil {
		to.AllowedOrigins = make(pq.StringArray, len(m.AllowedOrigins))
		copy(to.AllowedOrigins, m.AllowedOrigins)
	}
	to.StorageQuota = m.StorageQuota
	to.MonthlyRequestLimit = m.MonthlyRequestLimit
	to.MaxConcurrentJobs = m.MaxConcurrentJobs
	to.EnableCaching = m.EnableCaching
	to.EnableRateLimiting = m.EnableRateLimiting
	to.EnableRequestLogging = m.EnableRequestLogging
	for _, v := range m.Accounts {
		if v != nil {
			if tempAccounts, cErr := v.ToORM(ctx); cErr == nil {
				to.Accounts = append(to.Accounts, &tempAccounts)
			} else {
				return to, cErr
			}
		} else {
			to.Accounts = append(to.Accounts, nil)
		}
	}
	...
	to.TotalStorageUsed = m.TotalStorageUsed
	...
	to.Status = Status_name[int32(m.Status)]
	if posthook, ok := interface{}(m).(TenantWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *TenantORM) ToPB(ctx context.Context) (Tenant, error) {
	to := Tenant{}
	var err error
	if prehook, ok := interface{}(m).(TenantWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	for _, v := range m.Accounts {
		if v != nil {
			if tempAccounts, cErr := v.ToPB(ctx); cErr == nil {
				to.Accounts = append(to.Accounts, &tempAccounts)
			} else {
				return to, cErr
			}
	...
	to.Status = Status(Status_value[m.Status])
	if posthook, ok := interface{}(m).(TenantWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type Tenant the arg will be the target, the caller the one being converted from

// TenantBeforeToORM called before default ToORM code
type TenantWithBeforeToORM interface {
	BeforeToORM(context.Context, *TenantORM) error
}

// TenantAfterToORM called after default ToORM code
type TenantWithAfterToORM interface {
	AfterToORM(context.Context, *TenantORM) error
}

// TenantBeforeToPB called before default ToPB code
type TenantWithBeforeToPB interface {
	BeforeToPB(context.Context, *Tenant) error
}

// TenantAfterToPB called after default ToPB code
type TenantWithAfterToPB interface {
	AfterToPB(context.Context, *Tenant) error
}

type SubscriptionORM struct {
	AdvancedAnalyticsEnabled bool
	AdvancedFilteringEnabled bool
	...
	CurrentPeriodUsage       float32
	CustomExportsEnabled     bool
	CustomProxiesEnabled     bool
	EndedAt                  *time.Time
	Id                       uint64 `gorm:"primaryKey"`
	IncludedJobs             int32
	IncludedStorage          int64
	IsTrial                  bool
	MaxConcurrentJobs        int32
	MaxResultsPerJob         int32
	OrganizationId           *uint64
	...
	RetentionDays            int32
	StripeCustomerId         string `gorm:"index:idx_subscriptions_stripe_customer"`
	StripePriceId            string
	StripeProductId          string
	StripeSubscriptionId     string `gorm:"index:idx_subscriptions_stripe_subscription"`
	TrialEnd                 *time.Time
...
func (m *Subscription) ToORM(ctx context.Context) (SubscriptionORM, error) {
	...
	to.StripePriceId = m.StripePriceId
	to.StripeProductId = m.StripeProductId
	to.PlanTier = Subscription_PlanTier_name[int32(m.PlanTier)]
	to.BillingMode = Subscription_BillingMode_name[int32(m.BillingMode)]
	to.IncludedJobs = m.IncludedJobs
	to.PerJobRate = m.PerJobRate
	to.MaxConcurrentJobs = m.MaxConcurrentJobs
	to.IncludedStorage = m.IncludedStorage
	to.PerGbRate = m.PerGbRate
	to.AdvancedFilteringEnabled = m.AdvancedFilteringEnabled
	to.PrioritySupportEnabled = m.PrioritySupportEnabled
	to.CustomExportsEnabled = m.CustomExportsEnabled
	to.ApiAccessEnabled = m.ApiAccessEnabled
	to.CustomProxiesEnabled = m.CustomProxiesEnabled
	to.AdvancedAnalyticsEnabled = m.AdvancedAnalyticsEnabled
	to.RetentionDays = m.RetentionDays
	to.MaxResultsPerJob = m.MaxResultsPerJob
	if m.CurrentPeriodStart != nil {
		t := m.CurrentPeriodStart.AsTime()
		to.CurrentPeriodStart = &t
	}
	if m.CurrentPeriodEnd != nil {
		t := m.CurrentPeriodEnd.AsTime()
		to.CurrentPeriodEnd = &t
	}
	to.CurrentPeriodUsage = m.CurrentPeriodUsage
	to.Currency = m.Currency
	to.BasePrice = m.BasePrice
	to.BillingInterval = Subscription_Interval_name[int32(m.BillingInterval)]
	to.AutoRenew = m.AutoRenew
	to.PaymentStatus = Subscription_PaymentStatus_name[int32(m.PaymentStatus)]
	to.IsTrial = m.IsTrial
	if m.TrialStart != nil {
		t := m.TrialStart.AsTime()
		to.TrialStart = &t
	}
	if m.TrialEnd != nil {
		t := m.TrialEnd.AsTime()
...
func (m *SubscriptionORM) ToPB(ctx context.Context) (Subscription, error) {
	...
	to.StripePriceId = m.StripePriceId
	to.StripeProductId = m.StripeProductId
	to.PlanTier = Subscription_PlanTier(Subscription_PlanTier_value[m.PlanTier])
	to.BillingMode = Subscription_BillingMode(Subscription_BillingMode_value[m.BillingMode])
	to.IncludedJobs = m.IncludedJobs
	to.PerJobRate = m.PerJobRate
	to.MaxConcurrentJobs = m.MaxConcurrentJobs
	to.IncludedStorage = m.IncludedStorage
	...
	if m.CurrentPeriodEnd != nil {
		to.CurrentPeriodEnd = timestamppb.New(*m.CurrentPeriodEnd)
	}
	to.CurrentPeriodUsage = m.CurrentPeriodUsage
	to.Currency = m.Currency
	to.BasePrice = m.BasePrice
	to.BillingInterval = Subscription_Interval(Subscription_Interval_value[m.BillingInterval])
	to.AutoRenew = m.AutoRenew
	to.PaymentStatus = Subscription_PaymentStatus(Subscription_PaymentStatus_value[m.PaymentStatus])
	to.IsTrial = m.IsTrial
	if m.TrialStart != nil {
		to.TrialStart = timestamppb.New(*m.TrialStart)
	}
	if m.TrialEnd != nil {
		to.TrialEnd = timestamppb.New(*m.TrialEnd)
	}
	if m.CreatedAt != nil {
		to.CreatedAt = timestamppb.New(*m.CreatedAt)
	}
	...
	if m.CanceledAt != nil {
		to.CanceledAt = timestamppb.New(*m.CanceledAt)
	}
	if m.EndedAt != nil {
		to.EndedAt = timestamppb.New(*m.EndedAt)
	}
...
type WorkspaceORM struct {
	AccountId              *uint64
	ActiveIntegrations     pq.StringArray `gorm:"type:text[]"`
	ActiveScrapers         int32
	AlertOnBudgetThreshold bool
	AllowGuestAccess       bool
	AllowedEmailDomains    pq.StringArray `gorm:"type:text[]"`
	ApiKeys                []*APIKeyORM   `gorm:"foreignKey:WorkspaceId;references:Id"`
	BillingCurrency        string
	BudgetAlertThreshold   float32
	CompressExports        bool
	CompressionFormat      string
	CostPerLead            float32
	CreatedAt              *time.Time
	CurrentTeamMembers     int32
	DailyJobQuota          int32
	DeletedAt              *time.Time
	Description            string
	Domain                 string         `gorm:"index:idx_workspace_domain"`
	ExportDestinations     pq.StringArray `gorm:"type:text[]"`
	ExportFormats          pq.StringArray `gorm:"type:text[]"`
	ExportSchedule         string
	GdprCompliant          bool
	HipaaCompliant         bool
	HubspotConfig          string
	HubspotEnabled         bool
	Id                     uint64 `gorm:"primaryKey"`
	Industry               string
	JobsRunThisMonth       int32
	LastJobRun             *time.Time
	MaxConcurrentScrapes   int32
	MaxRetries             int32
	MaxTeamMembers         int32
	MonthlyBudget          float32
	Name                   string `gorm:"index:idx_workspace_name"`
	ProxyProvider          string
	ProxyRotationInterval  int32
	ProxyType              string
	ProxyUrls              pq.StringArray `gorm:"type:text[]"`
	RequestsPerSecond      int32
	RetryInterval          *time.Duration
	RotateProxies          bool
	SalesforceConfig       string
...
func (m *Workspace) ToORM(ctx context.Context) (WorkspaceORM, error) {
	...
	to.WorkspaceType = Workspace_WorkspaceType_name[int32(m.WorkspaceType)]
	to.Description = m.Description
	...
	to.MaxTeamMembers = m.MaxTeamMembers
	to.CurrentTeamMembers = m.CurrentTeamMembers
	to.AllowGuestAccess = m.AllowGuestAccess
	if m.AllowedEmailDomains != nil {
		to.AllowedEmailDomains = make(pq.StringArray, len(m.AllowedEmailDomains))
		copy(to.AllowedEmailDomains, m.AllowedEmailDomains)
	}
	if m.ProxyUrls != nil {
		to.ProxyUrls = make(pq.StringArray, len(m.ProxyUrls))
		copy(to.ProxyUrls, m.ProxyUrls)
	}
	to.ProxyType = m.ProxyType
	to.RotateProxies = m.RotateProxies
	to.ProxyRotationInterval = m.ProxyRotationInterval
	to.ProxyProvider = m.ProxyProvider
	// Repeated type ProxyAuthEntry is not an ORMable message type
	to.MaxConcurrentScrapes = m.MaxConcurrentScrapes
	...
	if m.ExportFormats != nil {
		to.ExportFormats = make(pq.StringArray, len(m.ExportFormats))
		copy(to.ExportFormats, m.ExportFormats)
	}
	to.ExportSchedule = m.ExportSchedule
	if m.ExportDestinations != nil {
		to.ExportDestinations = make(pq.StringArray, len(m.ExportDestinations))
		copy(to.ExportDestinations, m.ExportDestinations)
	}
	// Repeated type ExportDestinationConfigEntry is not an ORMable message type
	to.CompressExports = m.CompressExports
	to.CompressionFormat = m.CompressionFormat
	to.SalesforceEnabled = m.SalesforceEnabled
	to.SalesforceConfig = m.SalesforceConfig
	to.HubspotEnabled = m.HubspotEnabled
	to.HubspotConfig = m.HubspotConfig
	to.ZapierEnabled = m.ZapierEnabled
	to.ZapierWebhook = m.ZapierWebhook
	if m.ActiveIntegrations != nil {
		to.ActiveIntegrations = make(pq.StringArray, len(m.ActiveIntegrations))
		copy(to.ActiveIntegrations, m.ActiveIntegrations)
	}
	// Repeated type IntegrationSettingsEntry is not an ORMable message type
	to.MonthlyBudget = m.MonthlyBudget
	to.CostPerLead = m.CostPerLead
	to.AlertOnBudgetThreshold = m.AlertOnBudgetThreshold
	to.BudgetAlertThreshold = m.BudgetAlertThreshold
	to.BillingCurrency = m.BillingCurrency
...
func (m *WorkspaceORM) ToPB(ctx context.Context) (Workspace, error) {
	...
	to.WorkspaceType = Workspace_WorkspaceType(Workspace_WorkspaceType_value[m.WorkspaceType])
	to.Description = m.Description
	// Repeated type MetadataEntry is not an ORMable message type
	if m.Tags != nil {
		to.Tags = make(pq.StringArray, len(m.Tags))
...
type TenantAPIKeyORM struct {
	...
	Id          uint64 `gorm:"primaryKey"`
	KeyHash     string
	KeyPrefix   string `gorm:"index:idx_tenant_api_keys_prefix"`
	MaxUses     int32
	Name        string
	Status      string
	TenantId    *uint64
...
// TableName overrides the default tablename generated by GORM
func (TenantAPIKeyORM) TableName() string {
	return "tenant_api_keys"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *TenantAPIKey) ToORM(ctx context.Context) (TenantAPIKeyORM, error) {
	to := TenantAPIKeyORM{}
	var err error
	if prehook, ok := interface{}(m).(TenantAPIKeyWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(TenantAPIKeyWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *TenantAPIKeyORM) ToPB(ctx context.Context) (TenantAPIKey, error) {
	to := TenantAPIKey{}
	var err error
	if prehook, ok := interface{}(m).(TenantAPIKeyWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(TenantAPIKeyWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type TenantAPIKey the arg will be the target, the caller the one being converted from

// TenantAPIKeyBeforeToORM called before default ToORM code
type TenantAPIKeyWithBeforeToORM interface {
	BeforeToORM(context.Context, *TenantAPIKeyORM) error
}

// TenantAPIKeyAfterToORM called after default ToORM code
type TenantAPIKeyWithAfterToORM interface {
	AfterToORM(context.Context, *TenantAPIKeyORM) error
}

// TenantAPIKeyBeforeToPB called before default ToPB code
type TenantAPIKeyWithBeforeToPB interface {
	BeforeToPB(context.Context, *TenantAPIKey) error
}

// TenantAPIKeyAfterToPB called after default ToPB code
type TenantAPIKeyWithAfterToPB interface {
	AfterToPB(context.Context, *TenantAPIKey) error
}
...
func DefaultStrictUpdateOrganization(ctx context.Context, in *Organization, db *gorm.DB) (*Organization, error) {
	...
	filterSubscriptions := SubscriptionORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterSubscriptions.OrganizationId = new(uint64)
	*filterSubscriptions.OrganizationId = ormObj.Id
	if err = db.Where(filterSubscriptions).Delete(SubscriptionORM{}).Error; err != nil {
		return nil, err
	}
	filterTenants := TenantORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterTenants.OrganizationId = new(uint64)
	*filterTenants.OrganizationId = ormObj.Id
	if err = db.Where(filterTenants).Delete(TenantORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultApplyFieldMaskOrganization(ctx context.Context, patchee *Organization, patcher *Organization, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Organization, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"TechnicalEmail" {
			patchee.TechnicalEmail = patcher.TechnicalEmail
			continue
		}
		if f == prefix+"Phone" {
		...
		if f == prefix+"SsoEnabled" {
			patchee.SsoEnabled = patcher.SsoEnabled
			continue
		}
		if f == prefix+"SsoProvider" {
			patchee.SsoProvider = patcher.SsoProvider
			continue
		}
		if f == prefix+"SsoDomain" {
			patchee.SsoDomain = patcher.SsoDomain
			continue
		}
		...
		if f == prefix+"Enforce_2Fa" {
			patchee.Enforce_2Fa = patcher.Enforce_2Fa
			continue
		}
		...
		if f == prefix+"BillingCurrency" {
			patchee.BillingCurrency = patcher.BillingCurrency
			continue
		}
		if f == prefix+"AutoBilling" {
			patchee.AutoBilling = patcher.AutoBilling
			continue
		}
		if f == prefix+"TaxId" {
			patchee.TaxId = patcher.TaxId
			continue
		}
		if f == prefix+"ComplianceFrameworks" {
			patchee.ComplianceFrameworks = patcher.ComplianceFrameworks
			continue
		}
		if f == prefix+"DataProcessingAgreement" {
			patchee.DataProcessingAgreement = patcher.DataProcessingAgreement
			continue
		}
		if f == prefix+"DataRegion" {
			patchee.DataRegion = patcher.DataRegion
			continue
		}
		if f == prefix+"MaxTenants" {
			patchee.MaxTenants = patcher.MaxTenants
			continue
		}
		if f == prefix+"TotalStorageLimit" {
			patchee.TotalStorageLimit = patcher.TotalStorageLimit
			continue
		}
		if f == prefix+"MaxApiKeys" {
			patchee.MaxApiKeys = patcher.MaxApiKeys
			continue
		}
		if f == prefix+"MaxUsers" {
			patchee.MaxUsers = patcher.MaxUsers
			continue
		}
		if f == prefix+"Tenants" {
			patchee.Tenants = patcher.Tenants
			continue
		}
		if !updatedSubscriptions && strings.HasPrefix(f, prefix+"Subscriptions.") {
			updatedSubscriptions = true
			if patcher.Subscriptions == nil {
				patchee.Subscriptions = nil
				continue
			}
			if patchee.Subscriptions == nil {
				patchee.Subscriptions = &Subscription{}
			}
			if o, err := DefaultApplyFieldMaskSubscription(ctx, patchee.Subscriptions, patcher.Subscriptions, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Subscriptions.", db); err != nil {
				return nil, err
			} else {
				patchee.Subscriptions = o
			}
			continue
		}
...
// DefaultCreateTenant executes a basic gorm create call
func DefaultCreateTenant(ctx context.Context, in *Tenant, db *gorm.DB) (*Tenant, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TenantORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadTenant(ctx context.Context, in *Tenant, db *gorm.DB) (*Tenant, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := TenantORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(TenantORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TenantORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteTenant(ctx context.Context, in *Tenant, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&TenantORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(TenantORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type TenantORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteTenantSet(ctx context.Context, in []*Tenant, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&TenantORM{})).(TenantORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("id in (?)", keys).Delete(&TenantORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&TenantORM{})).(TenantORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type TenantORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*Tenant, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*Tenant, *gorm.DB) error
}

// DefaultStrictUpdateTenant clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateTenant(ctx context.Context, in *Tenant, db *gorm.DB) (*Tenant, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateTenant")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	...
	lockedRow := &TenantORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("id=?", ormObj.Id).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	filterAccounts := AccountORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterAccounts.TenantId = new(uint64)
	*filterAccounts.TenantId = ormObj.Id
	if err = db.Where(filterAccounts).Delete(AccountORM{}).Error; err != nil {
		return nil, err
	}
	filterApiKeys := TenantAPIKeyORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterApiKeys.TenantId = new(uint64)
	*filterApiKeys.TenantId = ormObj.Id
	if err = db.Where(filterApiKeys).Delete(TenantAPIKeyORM{}).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TenantORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchTenant executes a basic gorm update call with patch behavior
func DefaultPatchTenant(ctx context.Context, in *Tenant, updateMask *field_mask.FieldMask, db *gorm.DB) (*Tenant, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj Tenant
	var err error
	if hook, ok := interface{}(&pbObj).(TenantWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbReadRes, err := DefaultReadTenant(ctx, &Tenant{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	pbObj = *pbReadRes
	if hook, ok := interface{}(&pbObj).(TenantWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskTenant(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(TenantWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateTenant(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(TenantWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type TenantWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *Tenant, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type TenantWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *Tenant, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type TenantWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *Tenant, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type TenantWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *Tenant, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetTenant executes a bulk gorm update call with patch behavior
func DefaultPatchSetTenant(ctx context.Context, objects []*Tenant, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*Tenant, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*Tenant, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchTenant(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskTenant patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskTenant(ctx context.Context, patchee *Tenant, patcher *Tenant, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Tenant, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"ApiBaseUrl" {
			patchee.ApiBaseUrl = patcher.ApiBaseUrl
			continue
		}
		if f == prefix+"EnvironmentVariables" {
			patchee.EnvironmentVariables = patcher.EnvironmentVariables
			continue
		}
		if f == prefix+"AllowedOrigins" {
			patchee.AllowedOrigins = patcher.AllowedOrigins
			continue
		}
		...
		if f == prefix+"MonthlyRequestLimit" {
			patchee.MonthlyRequestLimit = patcher.MonthlyRequestLimit
			continue
		}
		if f == prefix+"MaxConcurrentJobs" {
			patchee.MaxConcurrentJobs = patcher.MaxConcurrentJobs
			continue
		}
		if f == prefix+"EnableCaching" {
			patchee.EnableCaching = patcher.EnableCaching
			continue
		}
		if f == prefix+"EnableRateLimiting" {
			patchee.EnableRateLimiting = patcher.EnableRateLimiting
			continue
		}
		if f == prefix+"EnableRequestLogging" {
			patchee.EnableRequestLogging = patcher.EnableRequestLogging
			continue
		}
		if f == prefix+"Accounts" {
			patchee.Accounts = patcher.Accounts
			continue
		}
		if f == prefix+"ApiKeys" {
		...
		if f == prefix+"TotalStorageUsed" {
			patchee.TotalStorageUsed = patcher.TotalStorageUsed
			continue
		}
...
// DefaultListTenant executes a gorm list call
func DefaultListTenant(ctx context.Context, db *gorm.DB) ([]*Tenant, error) {
	in := Tenant{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(TenantORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TenantORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
...
type TenantORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]TenantORM) error
}
...
func DefaultApplyFieldMaskSubscription(ctx context.Context, patchee *Subscription, patcher *Subscription, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Subscription, error) {
	...
	var updatedCurrentPeriodEnd bool
	var updatedTrialStart bool
	var updatedTrialEnd bool
	var updatedCreatedAt bool
	var updatedUpdatedAt bool
	var updatedCanceledAt bool
	var updatedEndedAt bool
	for i, f := range updateMask.Paths {
		if f == prefix+"Id" {
			patchee.Id = patcher.Id
			continue
		...
		if f == prefix+"StripePriceId" {
			patchee.StripePriceId = patcher.StripePriceId
			continue
		}
		if f == prefix+"StripeProductId" {
			patchee.StripeProductId = patcher.StripeProductId
			continue
		}
		if f == prefix+"PlanTier" {
			patchee.PlanTier = patcher.PlanTier
			continue
		}
		if f == prefix+"BillingMode" {
			patchee.BillingMode = patcher.BillingMode
			continue
		}
		if f == prefix+"IncludedJobs" {
			patchee.IncludedJobs = patcher.IncludedJobs
			continue
		}
		if f == prefix+"PerJobRate" {
			patchee.PerJobRate = patcher.PerJobRate
			continue
		}
		if f == prefix+"MaxConcurrentJobs" {
			patchee.MaxConcurrentJobs = patcher.MaxConcurrentJobs
		...
		if f == prefix+"IncludedStorage" {
			patchee.IncludedStorage = patcher.IncludedStorage
			continue
		}
		if f == prefix+"PerGbRate" {
			patchee.PerGbRate = patcher.PerGbRate
			continue
		}
		if f == prefix+"AdvancedFilteringEnabled" {
			patchee.AdvancedFilteringEnabled = patcher.AdvancedFilteringEnabled
			continue
		}
		if f == prefix+"PrioritySupportEnabled" {
			patchee.PrioritySupportEnabled = patcher.PrioritySupportEnabled
			continue
		}
		if f == prefix+"CustomExportsEnabled" {
			patchee.CustomExportsEnabled = patcher.CustomExportsEnabled
			continue
		}
		if f == prefix+"ApiAccessEnabled" {
			patchee.ApiAccessEnabled = patcher.ApiAccessEnabled
			continue
		}
		if f == prefix+"CustomProxiesEnabled" {
			patchee.CustomProxiesEnabled = patcher.CustomProxiesEnabled
			continue
		}
		if f == prefix+"AdvancedAnalyticsEnabled" {
			patchee.AdvancedAnalyticsEnabled = patcher.AdvancedAnalyticsEnabled
			continue
		}
		...
		if f == prefix+"MaxResultsPerJob" {
			patchee.MaxResultsPerJob = patcher.MaxResultsPerJob
			continue
		}
		...
		if !updatedCurrentPeriodEnd && strings.HasPrefix(f, prefix+"CurrentPeriodEnd.") {
			if patcher.CurrentPeriodEnd == nil {
				patchee.CurrentPeriodEnd = nil
				continue
			}
			if patchee.CurrentPeriodEnd == nil {
				patchee.CurrentPeriodEnd = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"CurrentPeriodEnd."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.CurrentPeriodEnd, patchee.CurrentPeriodEnd, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"CurrentPeriodEnd" {
			updatedCurrentPeriodEnd = true
			patchee.CurrentPeriodEnd = patcher.CurrentPeriodEnd
			continue
		}
		if f == prefix+"CurrentPeriodUsage" {
			patchee.CurrentPeriodUsage = patcher.CurrentPeriodUsage
			continue
		}
		...
		if f == prefix+"BasePrice" {
			patchee.BasePrice = patcher.BasePrice
			continue
		}
		if f == prefix+"BillingInterval" {
			patchee.BillingInterval = patcher.BillingInterval
			continue
		}
		if f == prefix+"AutoRenew" {
			patchee.AutoRenew = patcher.AutoRenew
			continue
		}
		if f == prefix+"PaymentStatus" {
			patchee.PaymentStatus = patcher.PaymentStatus
			continue
		}
		if f == prefix+"IsTrial" {
			patchee.IsTrial = patcher.IsTrial
			continue
		}
		if !updatedTrialStart && strings.HasPrefix(f, prefix+"TrialStart.") {
			if patcher.TrialStart == nil {
				patchee.TrialStart = nil
				continue
			}
			if patchee.TrialStart == nil {
				patchee.TrialStart = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"TrialStart."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.TrialStart, patchee.TrialStart, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"TrialStart" {
			updatedTrialStart = true
			patchee.TrialStart = patcher.TrialStart
			continue
		}
		if !updatedTrialEnd && strings.HasPrefix(f, prefix+"TrialEnd.") {
			if patcher.TrialEnd == nil {
				patchee.TrialEnd = nil
				continue
			}
			if patchee.TrialEnd == nil {
				patchee.TrialEnd = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"TrialEnd."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.TrialEnd, patchee.TrialEnd, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"TrialEnd" {
			updatedTrialEnd = true
			patchee.TrialEnd = patcher.TrialEnd
			continue
		}
		if !updatedCreatedAt && strings.HasPrefix(f, prefix+"CreatedAt.") {
			if patcher.CreatedAt == nil {
		...
		if !updatedCanceledAt && strings.HasPrefix(f, prefix+"CanceledAt.") {
			if patcher.CanceledAt == nil {
				patchee.CanceledAt = nil
				continue
			}
			if patchee.CanceledAt == nil {
				patchee.CanceledAt = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"CanceledAt."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.CanceledAt, patchee.CanceledAt, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"CanceledAt" {
			updatedCanceledAt = true
			patchee.CanceledAt = patcher.CanceledAt
			continue
		}
		if !updatedEndedAt && strings.HasPrefix(f, prefix+"EndedAt.") {
			if patcher.EndedAt == nil {
				patchee.EndedAt = nil
				continue
			}
			if patchee.EndedAt == nil {
				patchee.EndedAt = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"EndedAt."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.EndedAt, patchee.EndedAt, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"EndedAt" {
			updatedEndedAt = true
			patchee.EndedAt = patcher.EndedAt
			continue
		}
	}
	if err != nil {
...
func DefaultApplyFieldMaskWorkspace(ctx context.Context, patchee *Workspace, patcher *Workspace, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Workspace, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"WorkspaceType" {
			patchee.WorkspaceType = patcher.WorkspaceType
			continue
		}
		if f == prefix+"Description" {
			patchee.Description = patcher.Description
		...
		if f == prefix+"MaxTeamMembers" {
			patchee.MaxTeamMembers = patcher.MaxTeamMembers
			continue
		}
		if f == prefix+"CurrentTeamMembers" {
			patchee.CurrentTeamMembers = patcher.CurrentTeamMembers
			continue
		}
		if f == prefix+"AllowGuestAccess" {
			patchee.AllowGuestAccess = patcher.AllowGuestAccess
			continue
		}
		if f == prefix+"AllowedEmailDomains" {
			patchee.AllowedEmailDomains = patcher.AllowedEmailDomains
			continue
		}
		if f == prefix+"ProxyUrls" {
			patchee.ProxyUrls = patcher.ProxyUrls
			continue
		}
		if f == prefix+"ProxyType" {
			patchee.ProxyType = patcher.ProxyType
			continue
		}
		if f == prefix+"RotateProxies" {
			patchee.RotateProxies = patcher.RotateProxies
			continue
		}
		if f == prefix+"ProxyRotationInterval" {
			patchee.ProxyRotationInterval = patcher.ProxyRotationInterval
			continue
		}
		if f == prefix+"ProxyProvider" {
			patchee.ProxyProvider = patcher.ProxyProvider
			continue
		}
		if f == prefix+"ProxyAuth" {
			patchee.ProxyAuth = patcher.ProxyAuth
			continue
		}
		if f == prefix+"MaxConcurrentScrapes" {
			patchee.MaxConcurrentScrapes = patcher.MaxConcurrentScrapes
			continue
		}
		...
		if f == prefix+"ExportFormats" {
			patchee.ExportFormats = patcher.ExportFormats
			continue
		}
		if f == prefix+"ExportSchedule" {
			patchee.ExportSchedule = patcher.ExportSchedule
			continue
		}
		if f == prefix+"ExportDestinations" {
			patchee.ExportDestinations = patcher.ExportDestinations
			continue
		}
		if f == prefix+"ExportDestinationConfig" {
			patchee.ExportDestinationConfig = patcher.ExportDestinationConfig
			continue
		}
		if f == prefix+"CompressExports" {
			patchee.CompressExports = patcher.CompressExports
			continue
		}
		if f == prefix+"CompressionFormat" {
			patchee.CompressionFormat = patcher.CompressionFormat
			continue
		}
		if f == prefix+"SalesforceEnabled" {
			patchee.SalesforceEnabled = patcher.SalesforceEnabled
			continue
		}
		if f == prefix+"SalesforceConfig" {
			patchee.SalesforceConfig = patcher.SalesforceConfig
			continue
		}
		if f == prefix+"HubspotEnabled" {
			patchee.HubspotEnabled = patcher.HubspotEnabled
			continue
		}
		if f == prefix+"HubspotConfig" {
			patchee.HubspotConfig = patcher.HubspotConfig
			continue
		}
		if f == prefix+"ZapierEnabled" {
			patchee.ZapierEnabled = patcher.ZapierEnabled
			continue
		}
		if f == prefix+"ZapierWebhook" {
			patchee.ZapierWebhook = patcher.ZapierWebhook
			continue
		}
		if f == prefix+"ActiveIntegrations" {
			patchee.ActiveIntegrations = patcher.ActiveIntegrations
			continue
		}
		...
		if f == prefix+"MonthlyBudget" {
			patchee.MonthlyBudget = patcher.MonthlyBudget
			continue
		}
		if f == prefix+"CostPerLead" {
			patchee.CostPerLead = patcher.CostPerLead
			continue
		}
		if f == prefix+"AlertOnBudgetThreshold" {
			patchee.AlertOnBudgetThreshold = patcher.AlertOnBudgetThreshold
			continue
		}
		if f == prefix+"BudgetAlertThreshold" {
			patchee.BudgetAlertThreshold = patcher.BudgetAlertThreshold
			continue
		}
		if f == prefix+"BillingCurrency" {
			patchee.BillingCurrency = patcher.BillingCurrency
...
// DefaultCreateTenantAPIKey executes a basic gorm create call
func DefaultCreateTenantAPIKey(ctx context.Context, in *TenantAPIKey, db *gorm.DB) (*TenantAPIKey, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TenantAPIKeyORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadTenantAPIKey(ctx context.Context, in *TenantAPIKey, db *gorm.DB) (*TenantAPIKey, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := TenantAPIKeyORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(TenantAPIKeyORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TenantAPIKeyORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteTenantAPIKey(ctx context.Context, in *TenantAPIKey, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&TenantAPIKeyORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type TenantAPIKeyORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteTenantAPIKeySet(ctx context.Context, in []*TenantAPIKey, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&TenantAPIKeyORM{})).(TenantAPIKeyORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("id in (?)", keys).Delete(&TenantAPIKeyORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&TenantAPIKeyORM{})).(TenantAPIKeyORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type TenantAPIKeyORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*TenantAPIKey, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*TenantAPIKey, *gorm.DB) error
}

// DefaultStrictUpdateTenantAPIKey clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateTenantAPIKey(ctx context.Context, in *TenantAPIKey, db *gorm.DB) (*TenantAPIKey, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateTenantAPIKey")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	lockedRow := &TenantAPIKeyORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("id=?", ormObj.Id).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TenantAPIKeyORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchTenantAPIKey executes a basic gorm update call with patch behavior
func DefaultPatchTenantAPIKey(ctx context.Context, in *TenantAPIKey, updateMask *field_mask.FieldMask, db *gorm.DB) (*TenantAPIKey, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj TenantAPIKey
	var err error
	if hook, ok := interface{}(&pbObj).(TenantAPIKeyWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbReadRes, err := DefaultReadTenantAPIKey(ctx, &TenantAPIKey{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	pbObj = *pbReadRes
	if hook, ok := interface{}(&pbObj).(TenantAPIKeyWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskTenantAPIKey(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(TenantAPIKeyWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateTenantAPIKey(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(TenantAPIKeyWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type TenantAPIKeyWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *TenantAPIKey, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *TenantAPIKey, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *TenantAPIKey, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *TenantAPIKey, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetTenantAPIKey executes a bulk gorm update call with patch behavior
func DefaultPatchSetTenantAPIKey(ctx context.Context, objects []*TenantAPIKey, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*TenantAPIKey, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*TenantAPIKey, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchTenantAPIKey(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskTenantAPIKey patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskTenantAPIKey(ctx context.Context, patchee *TenantAPIKey, patcher *TenantAPIKey, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*TenantAPIKey, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"UseCount" {
			patchee.UseCount = patcher.UseCount
			continue
		}
		if !updatedExpiresAt && strings.HasPrefix(f, prefix+"ExpiresAt.") {
			if patcher.ExpiresAt == nil {
...
// DefaultListTenantAPIKey executes a gorm list call
func DefaultListTenantAPIKey(ctx context.Context, db *gorm.DB) ([]*TenantAPIKey, error) {
	in := TenantAPIKey{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TenantAPIKeyORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*TenantAPIKey{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type TenantAPIKeyORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type TenantAPIKeyORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]TenantAPIKeyORM) error
}
