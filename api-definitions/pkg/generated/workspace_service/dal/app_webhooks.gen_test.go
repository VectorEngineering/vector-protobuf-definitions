...
var AppWebhookORMCreateInBatchTestCase = []TestCase{}

func Test_appWebhookORM_CreateInBatch(t *testing.T) {
	appWebhookORM := newAppWebhookORM(_gen_test_db)
	do := appWebhookORM.WithContext(context.Background()).Debug()

	for i, tt := range AppWebhookORMCreateInBatchTestCase {
		t.Run("CreateInBatch_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateInBatch(tt.Input.Args[0].([]workspace_servicev1.AppWebhookORM), tt.Input.Args[1].(int))
