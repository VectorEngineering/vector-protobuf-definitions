...
var DocumentVersionORMCreateInBatchTestCase = []TestCase{}

func Test_documentVersionORM_CreateInBatch(t *testing.T) {
	documentVersionORM := newDocumentVersionORM(_gen_test_db)
	do := documentVersionORM.WithContext(context.Background()).Debug()

	for i, tt := range DocumentVersionORMCreateInBatchTestCase {
		t.Run("CreateInBatch_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateInBatch(tt.Input.Args[0].([]workspace_servicev1.DocumentVersionORM), tt.Input.Args[1].(int))
