...
var RiskAssessmentORMCreateInBatchTestCase = []TestCase{}

func Test_riskAssessmentORM_CreateInBatch(t *testing.T) {
	riskAssessmentORM := newRiskAssessmentORM(_gen_test_db)
	do := riskAssessmentORM.WithContext(context.Background()).Debug()

	for i, tt := range RiskAssessmentORMCreateInBatchTestCase {
		t.Run("CreateInBatch_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateInBatch(tt.Input.Args[0].([]workspace_servicev1.RiskAssessmentORM), tt.Input.Args[1].(int))
