...
var SignatureWorkflowORMCreateInBatchTestCase = []TestCase{}

func Test_signatureWorkflowORM_CreateInBatch(t *testing.T) {
	signatureWorkflowORM := newSignatureWorkflowORM(_gen_test_db)
	do := signatureWorkflowORM.WithContext(context.Background()).Debug()

	for i, tt := range SignatureWorkflowORMCreateInBatchTestCase {
		t.Run("CreateInBatch_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateInBatch(tt.Input.Args[0].([]workspace_servicev1.SignatureWorkflowORM), tt.Input.Args[1].(int))
