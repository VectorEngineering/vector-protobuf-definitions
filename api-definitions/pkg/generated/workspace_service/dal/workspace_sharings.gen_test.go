...
var WorkspaceSharingORMCreateInBatchTestCase = []TestCase{}

func Test_workspaceSharingORM_CreateInBatch(t *testing.T) {
	workspaceSharingORM := newWorkspaceSharingORM(_gen_test_db)
	do := workspaceSharingORM.WithContext(context.Background()).Debug()

	for i, tt := range WorkspaceSharingORMCreateInBatchTestCase {
		t.Run("CreateInBatch_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateInBatch(tt.Input.Args[0].([]workspace_servicev1.WorkspaceSharingORM), tt.Input.Args[1].(int))
