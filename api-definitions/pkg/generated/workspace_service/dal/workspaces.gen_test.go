...
func init() {
	InitializeDB()
	err := _gen_test_db.AutoMigrate(&workspace_servicev1.WorkspaceORM{})
	if err != nil {
...
func Test_workspaceORM_CreateInBatch(t *testing.T) {
	...
	for i, tt := range WorkspaceORMCreateInBatchTestCase {
		t.Run("CreateInBatch_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateInBatch(tt.Input.Args[0].([]workspace_servicev1.WorkspaceORM), tt.Input.Args[1].(int))
