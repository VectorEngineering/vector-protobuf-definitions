...
type FolderMetadataORM struct {
	ChildFolders     []*FolderMetadataORM `gorm:"foreignKey:FolderMetadataId;references:Id"`
	CreatedAt        *time.Time
	DeletedAt        *time.Time
...
func (m *FolderMetadata) ToORM(ctx context.Context) (FolderMetadataORM, error) {
	...
	for _, v := range m.ChildFolders {
		if v != nil {
			if tempChildFolders, cErr := v.ToORM(ctx); cErr == nil {
				to.ChildFolders = append(to.ChildFolders, &tempChildFolders)
			} else {
				return to, cErr
			}
		} else {
			to.ChildFolders = append(to.ChildFolders, nil)
		}
	}
...
func (m *FolderMetadataORM) ToPB(ctx context.Context) (FolderMetadata, error) {
	...
	for _, v := range m.ChildFolders {
		if v != nil {
			if tempChildFolders, cErr := v.ToPB(ctx); cErr == nil {
				to.ChildFolders = append(to.ChildFolders, &tempChildFolders)
			} else {
				return to, cErr
			}
...
func DefaultStrictUpdateFolderMetadata(ctx context.Context, in *FolderMetadata, db *gorm.DB) (*FolderMetadata, error) {
	...
	filterChildFolders := FolderMetadataORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterChildFolders.FolderMetadataId = new(uint64)
	*filterChildFolders.FolderMetadataId = ormObj.Id
	if err = db.Where(filterChildFolders).Delete(FolderMetadataORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultApplyFieldMaskFolderMetadata(ctx context.Context, patchee *FolderMetadata, patcher *FolderMetadata, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*FolderMetadata, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"ChildFolders" {
			patchee.ChildFolders = patcher.ChildFolders
			continue
		}
		if f == prefix+"Files" {
