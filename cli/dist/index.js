      ...
      */
      ...
      _executeSubCommand(subcommand, args) {
        ...
        if (!exitCallback) {
          ...
          proc.on("close", () => {
            exitCallback(new CommanderError(process8.exitCode || 0, "commander.executeSubCommandAsync", "(close)"));
          });
        }
        proc.on("error", (err) => {
          if (err.code === "ENOENT") {
...
var import_node_process5, import_node_readline, import_bl, ASCII_ETX_CODE, StdinDiscarder, stdinDiscarder, stdin_discarder_default;
...
var import_node_process6, import_cli_spinners, import_cli_spinners2, Ora;
var init_ora = __esm({
  "node_modules/ora/index.js"() {
    "use strict";
...
async function generate(options) {
  ...
  try {
    const openApiContent = await (0, import_promises.readFile)(options.input, "utf-8");
    const openApiSpec = JSON.parse(openApiContent);
    const { zodSchemas, apiClient } = await (0, import_openapi_zod_client.createZodClientFromOpenAPI)({
      openApiDoc: openApiSpec,
      options: {
        withAlias: true,
        defaultStatusBehavior: "success"
      }
    });
    await (0, import_promises.mkdir)(options.output, { recursive: true });
    const templateDir = options.template || (0, import_path.join)(process.cwd(), "templates");
    const templateFiles = await (0, import_glob.glob)("**/*.hbs", { cwd: templateDir });
    for (const templateFile of templateFiles) {
      const templatePath = (0, import_path.join)(templateDir, templateFile);
      const templateContent = await (0, import_promises.readFile)(templatePath, "utf-8");
      const template = (0, import_handlebars.compile)(templateContent);
      ...
      await (0, import_promises.writeFile)(outputPath, rendered);
    }
    spinner.succeed(source_default.green("API client generated successfully"));
  } catch (error) {
    spinner.fail(source_default.red("Failed to generate API client"));
    console.error(error);
    process.exit(1);
  }
}
var import_path, import_promises, import_handlebars, import_openapi_zod_client, import_glob;
var init_generate = __esm({
  "src/commands/generate.ts"() {
    ...
    init_source();
    import_handlebars = require("handlebars");
    import_openapi_zod_client = require("openapi-zod-client");
...
var { generate: generate2 } = (init_generate(), __toCommonJS(generate_exports));
var chalk2 = (init_source(), __toCommonJS(source_exports));
var program = new Command();
program.name("vector").description("Vector API CLI tool").version("0.1.0");
program.command("generate").description("Generate API client code from OpenAPI specification").requiredOption("-i, --input <path>", "Path to OpenAPI specification file").requiredOption("-o, --output <path>", "Output directory for generated code").option("-t, --template <path>", "Path to custom templates directory").action(generate2);
program.parse(process.argv);
