...
export async function generate(options: GenerateOptions) {
  ...
  try {
    ...
    const rateLimitTemplate = await readFile(
      join(templateDir, 'middleware', 'rateLimit.hbs'),
      'utf-8'
    );
    await writeFile(join(middlewareDir, 'rateLimit.ts'), rateLimitTemplate);

    const authTemplate = await readFile(
      join(templateDir, 'middleware', 'auth.hbs'),
      'utf-8'
    );
    await writeFile(join(middlewareDir, 'auth.ts'), authTemplate);

    const middlewareIndexTemplate = await readFile(
      join(templateDir, 'middleware', 'index.hbs'),
      'utf-8'
    );
