 ...
 */
export function extractContentSections(html: string): Array<{ heading: string; content: string }> {
    const sections: Array<{ heading: string; content: string }> = [];
    const headingPattern = /<h[1-6][^>]*>(.*?)<\/h[1-6]>([\s\S]*?)(?=<h[1-6]|$)/gi;
 ...
 */
export function extractLists(html: string): { ordered: string[][]; unordered: string[][] } {
    const lists = {
        ordered: [] as string[][],
        unordered: [] as string[][]
    };
    ...
    while ((olMatch = olPattern.exec(html)) !== null) {
        ...
        lists.ordered.push(items.map(item => item.replace(/<[^>]+>/g, '').trim()));
    }
    ...
    while ((ulMatch = ulPattern.exec(html)) !== null) {
        ...
        lists.unordered.push(items.map(item => item.replace(/<[^>]+>/g, '').trim()));
    }
...
export function extractTables(html: string): Array<{ headers: string[]; rows: string[][] }> {
    ...
    while ((tableMatch = tablePattern.exec(html)) !== null) {
        ...
        while ((rowMatch = rowPattern.exec(tableMatch[1])) !== null) {
            ...
            table.rows.push(cells.map(cell =>
                cell.replace(/<[^>]+>/g, '').trim()
            ));
        }
