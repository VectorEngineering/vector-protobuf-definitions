...
export function sanitizeHtml(html: string): string {
    ...
    const allowedTags = [
        ...
        'ul', 'ol', 'li', 'a', 'img', 'blockquote', 'em', 'strong',
        'b', 'i', 'code', 'pre', 'br', 'hr', 'table', 'tr', 'td', 'th'
    ];
    ...
    sanitized = sanitized.replace(attrPattern, (match, prefix) => {
        const attrs = match.slice(prefix.length, -1).match(/([a-z-]+)(?:="[^"]*")?/gi) || [];
 ...
 */
export function extractMetadata(html: string): Record<string, string | string[]> {
...
export function extractMainContent(html: string): string {
    ...
    for (const selector of mainContentSelectors) {
        const match = html.match(new RegExp(`<${selector}[^>]*>(.*?)<\/${selector}>`, 'is'));
...
export function htmlToMarkdown(html: string): string {
    ...
    // Replace headers
    markdown = markdown.replace(/<h[1-6][^>]*>(.*?)<\/h[1-6]>/gi, (_, content) => `\n# ${content.trim()}\n`);

    // Replace paragraphs
    markdown = markdown.replace(/<p[^>]*>(.*?)<\/p>/gi, (_, content) => `\n${content.trim()}\n`);

    // Replace links
    markdown = markdown.replace(/<a[^>]*href="([^"]*)"[^>]*>(.*?)<\/a>/gi, (_, url, text) => `[${text.trim()}](${url})`);

    // Replace lists
    markdown = markdown.replace(/<li[^>]*>(.*?)<\/li>/gi, (_, content) => `- ${content.trim()}\n`);
