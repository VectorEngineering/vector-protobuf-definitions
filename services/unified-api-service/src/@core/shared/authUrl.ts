...
export const constructAuthUrl = async ({
  ...
  if (
    config.options?.local_redirect_uri_in_https &&
    redirectUriIngress?.status
  ) {
    baseRedirectURL = redirectUriIngress.value!;
  }

  const encodedRedirectUrl = encodeURIComponent(
    `${baseRedirectURL}/connections/oauth/callback`,
  );
...
const handleOAuth2Url = async ({
  ...
  const response = await fetch(
    `${apiUrl}/connection_strategies/getCredentials?projectId=${projectId}&type=${type}`,
  );
  ...
  if (needsSubdomain(providerName, vertical)) {
    BASE_URL =
      typeof baseUrl === 'string'
        ? baseUrl
        : (baseUrl as DynamicAuthorization)(data.SUBDOMAIN as string);
  } else if (needsEndUserSubdomain(providerName, vertical)) {
    BASE_URL =
      typeof baseUrl === 'string'
        ? baseUrl
        : (baseUrl as DynamicAuthorization)(...Object.values(dyn || {}));
  } else {
  ...
  let params = `response_type=code&client_id=${encodeURIComponent(
    clientId,
  )}&redirect_uri=${encodedRedirectUrl}&state=${state}`;
  ...
  switch (providerName) {
    ...
    case 'squarespace':
      ...
      break;
    case 'faire':
      params = `applicationId=${encodeURIComponent(
        clientId,
      )}&redirectUrl=${encodedRedirectUrl}&state=${state}`;
      break;
    case 'ebay':
      params = `response_type=code&client_id=${encodeURIComponent(
        clientId,
      )}&redirect_uri=${data.RUVALUE}&state=${state}`;
      break;
    case 'amazon':
      params = `application_id=${encodeURIComponent(
        data.APPLICATION_ID,
