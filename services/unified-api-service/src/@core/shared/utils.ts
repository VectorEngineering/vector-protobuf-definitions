...
function getActiveProvidersForVertical(vertical: string): VerticalConfig {
  ...
  for (const [providerName, config] of Object.entries(verticalConfig)) {
    if (config.active !== false) {
      // Assuming undefined or true means active
  ...
  if (vertical) {
    ...
    return Object.entries(activeProviders).map(([providerName, config]) => ({
      ...
      authStrategy: {
        ...
        properties: config.authStrategy.properties
          ? config.authStrategy.properties
          : [],
      },
    }));
  } else {
    ...
    categoriesVerticals.forEach((vertical) => {
      const activeProviders = getActiveProvidersForVertical(vertical);
      const providersForVertical = Object.entries(activeProviders).map(
        ([providerName, config]) => ({
          vertical: vertical.toLowerCase(),
          name: providerName,
          urls: {
...
export function findProviderByName(providerName: string): Provider | null {
  for (const vertical in CONNECTORS_METADATA) {
    if (
      CONNECTORS_METADATA.hasOwnProperty.call(CONNECTORS_METADATA, vertical)
    ) {
      const activeProviders = getActiveProvidersForVertical(vertical);
...
export const ALL_PROVIDERS: { vertical: string; value: string }[] =
  mergeAllProviders(
    CRM_PROVIDERS,
    ACCOUNTING_PROVIDERS,
    TICKETING_PROVIDERS,
