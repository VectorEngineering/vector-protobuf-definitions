    ...
    const result: LinearTicketInput = {
      ...
      project: {
        id: source.collections
          ? await this.utils.getCollectionRemoteIdFromUuid(
              source.collections[0] as string,
            )
          : null,
      },
      team: {
        id: await this.utils.getTeamRemoteIdFromUuid(
          source.field_mappings['team_id'],
        ),
      },
  ...
  private async mapSingleTicketToUnified(
    ...
    if (ticket.state) {
      opts = {
        ...opts,
        type:
          ticket.state.name === 'Canceled' || ticket.state.name === 'Done'
            ? 'CLOSED'
            : 'OPEN',
      };
    }
